...
WPD-QA 是一个面向农业领域的小麦病虫害智能问答与学习平台，包含移动端应用和管理后台两部分。移动端应用为农户、农技人员和农业学生提供 AI 智能问答、RAG 知识库检索、农业知识学习、智能答题训练等功能，帮助用户快速获取病虫害知识、防治方案及农业种植建议；管理后台用于管理员进行知识库维护、用户管理和系统监控。
...
```
WPD-QA/
├── frontend/                  # 移动端应用（Next.js） - 响应式设计，支持手机和平板
│   ├── src/
│   │   ├── app/            # 页面路由
│   │   │   ├── chat/       # AI问答页
│   │   │   ├── quiz/       # 知识答题页
